...
export class Table extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      filter: "",
      dev: window.location.href.includes("quiq.dev")
    };
      ...
      function(column, i) {
        if (typeof column === "string") {
          ...
          if (typeof column.sortable !== "undefined") {
            let sortFunction =
              column.sortable === true ? "default" : column.sortable;
    ...
    if (typeof props.children !== "undefined") {
      React.Children.forEach(
        ...
        function(child) {
          ...
          switch (this.state.dev ? `${child.type}` : child.type) {
            case this.state.dev ? `${Thead}` : Thead:
              break;
            case this.state.dev ? `${Tfoot}` : Tfoot:
              if (typeof tfoot !== "undefined") {
                console.warn(
                  "You can only have one <Tfoot>, but more than one was specified." +
                    "Ignoring all but the last one"
                );
              }
              tfoot = child;
              break;
            case this.state.dev ? `${Tr}` : Tr:
              let childData = child.props.data || {};

              React.Children.forEach(child.props.children, function(
                descendant
              ) {
                // TODO
                ...
                if (typeof descendant !== "object" || descendant == null) {
                  ...
                  console.warn(
                    "exports.Td specified without a " +
            ...
            default:
              console.warn(
                "The only possible children of <Table> are <Thead>, <Tr>, " +
                  "or one <Tfoot>."
              );
          }
        }.bind(this)
    ...
    if (column instanceof Object) {
      ...
      if (typeof column.direction !== "undefined") {
        if (column.direction === 1 || column.direction === "asc") {
          ...
          let defaultDirection = this.props.defaultSortDescending
            ? "descending"
            : "ascending";

          console.warn(
            "Invalid default sort specified. Defaulting to " + defaultDirection
          );
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      for (let filterColumn in this._filterable) {
        if (typeof data[filterColumn] !== "undefined") {
          // Default filter
          if (
            typeof this._filterable[filterColumn] === "undefined" ||
            this._filterable[filterColumn] === "default"
          ) {
            if (
              extractDataFrom(data, filterColumn)
                .toString()
                .toLowerCase()
                .indexOf(filter) > -1
            ) {
            ...
            // Apply custom filter
            if (
              this._filterable[filterColumn](
                extractDataFrom(data, filterColumn).toString(),
                filter
              )
            ) {
              matchedChildren.push(children[i]);
    ...
    return (
      <table {...props}>
        {this.state.dev && (
          ...
          >
            Table Headers will look weird only while developing locally
          </div>
        )}
        {tableHeader}
